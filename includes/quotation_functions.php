...
function ensureQuotationSchema(PDO $pdo) {
    try {
        $pdo->exec("
        ...
        ensureQuotationPermission($pdo);
    } catch (Exception $e) {
        error_log('quotation schema migration failed: ' . $e->getMessage());
    }
}

function ensureQuotationPermission(PDO $pdo) {
    try {
        ...
        if (isset($_SESSION['admin_permissions']) && is_array($_SESSION['admin_permissions']) && !in_array('manage_quotations', $_SESSION['admin_permissions'], true)) {
            $_SESSION['admin_permissions'][] = 'manage_quotations';
        }
    } catch (Exception $e) {
        error_log('quotation permission migration failed: ' . $e->getMessage());
    }
}

function generateQuotationNumber(PDO $pdo) {
    $prefix = 'QTN-' . date('Ym') . '-';
    $stmt = $pdo->prepare("SELECT quotation_no FROM quotations WHERE quotation_no LIKE ? ORDER BY id DESC LIMIT 1");
    $stmt->execute([$prefix . '%']);
...
function getQuotationProducts(PDO $pdo) {
...
function getQuotations(PDO $pdo, array $filters = []) {
    $where = [];
    $params = [];

    if (!empty($filters['month'])) {
        $where[] = "DATE_FORMAT(quotation_date, '%Y-%m') = ?";
    ...
    if (!empty($filters['search'])) {
        $where[] = "(quotation_no LIKE ? OR customer_name LIKE ? OR mobile_no LIKE ? OR bill_to_gstin LIKE ? OR ship_to_gstin LIKE ?)";
    ...
    $stmt = $pdo->prepare("
        ...
        FROM quotations mi
        LEFT JOIN quotation_items mii ON mi.id = mii.quotation_id
        $whereSql
        GROUP BY mi.id
...
function getQuotationById(PDO $pdo, $quotationId) {
    $stmt = $pdo->prepare("SELECT * FROM quotations WHERE id = ?");
    $stmt->execute([(int)$quotationId]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}

function getQuotationItems(PDO $pdo, $quotationId) {
    $stmt = $pdo->prepare("SELECT * FROM quotation_items WHERE quotation_id = ? ORDER BY sort_order ASC, id ASC");
    $stmt->execute([(int)$quotationId]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

function deleteQuotation(PDO $pdo, $quotationId) {
    $stmt = $pdo->prepare("DELETE FROM quotations WHERE id = ?");
    return $stmt->execute([(int)$quotationId]);
}

function calculateQuotationItem(array $item) {
...
function QuotationAmountWords($number) {
...
function QuotationImageDataUri($relativePath) {
