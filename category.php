...
.products-grid {
  display: grid;
  grid-template-columns: repeat(3, 1fr); /* Tablet/Desktop Default: 3 cards per row to fit with sidebar */
  gap: 20px;
  margin-bottom: 40px;
  width: 100%;
  max-width: 100%;
...
.product-card {
  ...
  min-width: 0 !important; /* Override fixed 240px from product-card.css */
  overflow: hidden;
...
@media (max-width: 1200px) {
  .products-grid {
    grid-template-columns: repeat(2, 1fr); /* Smaller desktop/Tablet: 2 cards per row */
    gap: 15px;
  }
}
